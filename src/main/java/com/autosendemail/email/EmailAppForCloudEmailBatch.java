...
@Service
public class EmailAppForCloudEmailBatch{
    ...
    @Async
    public void sendEmailBatchManyThread(){
        while (true){
            ...
            if("1".equals(is_open)) {
                ...
                if(fact_get_count > 0){
                    ...
                    //更新FACT_SEND_TEMP_COUNT字段
                    EmailSourceBean sourceBean_temp = new EmailSourceBean();
                    sourceBean_temp.setPlan_date(datestr);
                    sourceBean_temp.setSend_source(sendway);
                    emailDbOperateForSend.upDateSourceControlFactCountTemp(sourceBean_temp,String.valueOf(toEmails.size()),sendway);
                    ...
                    if (toEmails != null && toEmails.size() > 0) {
                        ...
                        for (Map emaiinfoArrdT : toEmails) {
                            EmailSendAddrBean emailSendAddrBean = new EmailSendAddrBean();
                            emailSendAddrBean.setEmailAddr(emaiinfoArrdT.get("EMAIL_ADDR").toString());
                            //emailSendAddrBean.setEmailAddr("golden1@hd2.dot.net.in");
                            System.out.println("begin-----------------");
                            emailBatchInfoService.getSendAddrInfo(emailSendAddrBean);
                        ...
                        for (Map emaiinfoArrd : toEmails) {
                            //处理多线程读取同一条记录情况
                            Map mapinfo = emailDbOperateForSend.getEmailInfoOne(emaiinfoArrd.get("EMAIL_ADDR").toString());
                            if(mapinfo != null){
                                String sendstatus = mapinfo.get("SEND_STATUS").toString();
                                if(!"03".equalsIgnoreCase(sendstatus)){
                                    continue;
                                }
                            }
                            String apikey = "";
                            ...
                            List<Map<String, Object>> list = emailDbOperateForSend.getAllApiKey(EmailSendServerType.SENDGRID.getTranCode());
                            if(list == null){
                                break;
                            }
